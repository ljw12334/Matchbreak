<link rel="stylesheet" href="/matchbreak/css/calendar.css">

<?php function set_resdays()
{ // Reservation days, 예약 일자
	$link = "/matchbreak/index.php";
	/*
...
<?php function set_calendar_backup($date = 0)
{
	if (!$date) {
		$date = date('Y-m-d', time());
	}
...
<?php function set_resdays2($link = "/matchbreak/index.php")
{ // Reservation days, 예약 일자
	/*
			 현재 날짜를 가져와서 앞으로 10일간의 날짜를 계산해
			 [YYYY-mm-dd, 요일] 의 형태로 리스트에 추가한다
	...
	for ($i = 0; $i < 10; $i++) {
		...
		$days = array([$day, $date_of_week]); // 배열 마지막에 요소 추가
