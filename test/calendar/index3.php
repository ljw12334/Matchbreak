...
<html lang="ko">
    ...
    <body>
        ...
        <?php function set_calendar($date = 0) {
        	...
        	$start_week = date('w',strtotime($now_year.'-'.$now_month.'-01'));
        	$total_week = ceil(($max_day+$start_week)/7);
        	$last_week = date('w',strtotime($now_year.'-'.$now_month.'-'.$max_day));
        	$before_max_day = date('t',strtotime($now_year.'-'.$now_month.' -1 month'));
        	$after_min_day = 1;
        			...
        			<div>
        				<?php for($j = 0; $j < 7; $j++){ ?>
        					...
        					<?php if(!(($i == 0 && $j < $start_week) || (($i+1) == $total_week && $j > $last_week))){
								$today = $now_day == $day ? " class='today'" : "";

								if ($day < $now_day) {
									echo "<div".$add_class."><span class='past-day'>".$day++."</span></div>";
								} else {
